...
#[test]
fn test_double_nat() {
    let engine = Engine::new().with_fetcher(NoImports).with_builtin("doubleNat", DoubleNat);
    assert_eq!(engine.eval_str("doubleNat 21").unwrap().to_string(), "42");
}

#[test]
fn test_hash_builder() {
    let engine = Engine::new().with_fetcher(NoImports).with_builtin("hashBuilder", HashBuilder);
