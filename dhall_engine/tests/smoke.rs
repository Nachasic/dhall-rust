...
const DOUBLE_NAT_TYPE: &str = "Natural -> Natural";
const HASH_BUILDER_TYPE: &str =
...
fn test_double_nat() {
    let engine = Engine::new()
        ...
        .with_builtin("doubleNat", DOUBLE_NAT_TYPE, DoubleNat);
...
fn test_hash_builder() {
    let engine = Engine::new()
        .with_fetcher(NoImports)
        .with_builtin("hashBuilder", HASH_BUILDER_TYPE, HashBuilder);
...
fn test_type_error_wrong_arg() {
    let engine = Engine::new()
        .with_fetcher(NoImports)
        .with_builtin("doubleNat", DOUBLE_NAT_TYPE, DoubleNat);
    // doubleNat expects Natural, not Text
    let result = engine.eval_str(r#"doubleNat "hello""#);
    assert!(result.is_err(), "expected type error, got: {:?}", result);
}

#[test]
fn test_type_error_wrong_record_shape() {
    let engine = Engine::new()
        .with_fetcher(NoImports)
        .with_builtin("hashBuilder", HASH_BUILDER_TYPE, HashBuilder);
    // hashBuilder expects { name : Text, src : Text }, not { x : Natural }
    let result = engine.eval_str("hashBuilder { x = 1 }");
    assert!(result.is_err(), "expected type error, got: {:?}", result);
}
